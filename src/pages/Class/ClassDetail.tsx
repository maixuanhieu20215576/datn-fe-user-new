    ...
    const fetchClassDetail = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/admin/fetch-class/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleRegister = async () => {
    try {
      ...
      const response = await axios.post(
        ...
        {
          price:
            classData.priceType === constants.priceType.byDay
              ? classData.price * classData.schedule.length
              : classData.price,
          userId,
          classId: id,
        }
  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
          ...
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-3">
              <p className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                ...
                <Link
                  to={`/teacher-profile/${classData.teacherId}`}
              ...
              <p className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                ...
                {formatNumber(classData.price)}đ/
                {classData.priceType === constants.priceType.byDay
                  ? "buổi"
                  : "khóa"}
              </p>
              ...
              <p className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                ...
                {formatNumber(classData.currentStudent)}/
                {classData.maxStudent
                  ? formatNumber(classData.maxStudent)
                  : "Không giới hạn"}
              </p>
        ...
        <ClassDescription
          language={
            classData.language.charAt(0).toUpperCase() +
            classData.language.slice(1).toLowerCase()
          }
        />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {classData.schedule.length > 0 && (
              <div className="col-span-full text-gray-700 dark:text-gray-300">
                ...
                <p>
                  Đến ngày:{" "}
                  {classData.schedule[classData.schedule.length - 1].date}
                </p>
              </div>
            )}
            {classData.stringForDisplayScheduleByDayOfWeeks.map(
              (stringForDisplayScheduleByDayOfWeek, index) => (
                <div
                  key={index}
                  className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow"
                >
                  <p className="text-gray-700 dark:text-gray-300">
                    {stringForDisplayScheduleByDayOfWeek}
                  </p>
                </div>
              )
            )}
        ...
        <div className="mt-6 flex justify-center">
          <button
            onClick={() =>
              isRegistered ? navigate(`/your-class/${id}`) : setShowModal(true)
            }
