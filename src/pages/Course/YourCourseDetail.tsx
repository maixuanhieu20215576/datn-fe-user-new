...
const CourseContent = () => {
    ...
    useEffect(() => {
        const fetchCourseUnit = async () => {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/course/get-course-unit/${courseId.id}`);
    ...
    return (
        <div className="max-w-3xl mx-auto py-8">
            <h1 className="text-3xl font-semibold mb-4 dark:text-white">{course?.course_name}</h1>
            {course?.units.map((unit, idx) => (
                <AccordionItem key={idx} title={unit.unitName}>
                    <div className="space-y-2">
                        {unit.unitContent.map((content, cidx) => (
                            <div key={cidx} className="text-gray-700 hover:text-gray-900 cursor-pointer px-4 py-2 hover:bg-gray-100 rounded-md dark:text-white dark:hover:bg-gray-700" onClick={() => navigate(`/your-course/${courseId.id}/unit/${content._id}`)}>
                                {content.childUnitName}
                            </div>
                        ))}
                    </div>
                </AccordionItem>
    ...
    return (
        <div className="bg-white shadow-md p-4">
            ...
            <div className="mt-6">
                ...
                <p className="text-sm text-gray-600">Sau thời gian này, khóa học sẽ chuyển sang trạng thái lưu trữ.</p>
            </div>
        </div>
    );
};
