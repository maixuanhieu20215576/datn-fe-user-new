...
export default function TeacherDashboard() {
  ...
  const [teachingStatisticsByClass, setTeachingStatisticsByClass] = useState<
    TeachingStatisticsByClass[]
  >([]);
    ...
    const fetchDashboardData = async () => {
      try {
        const response = await axios.get(
          ...
          }/teacher/statistics/${userId}?timePeriod=${timePeriod}`,
          {
            headers: {
    ...
    const fetchTeachingStatisticsByClass = async () => {
      try {
        const response = await axios.get(
          `${
            import.meta.env.VITE_API_URL
          }/teacher/statistics-by-class/${userId}?timePeriod=${timePeriod}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {teachingStatisticsByClass.map((cls) => (
                <tr key={cls._id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                    {cls.totalStudents}/
                    {cls.maxStudents ? cls.maxStudents : "Không giới hạn"}
                  </td>
                </tr>
              ))}
            </tbody>
